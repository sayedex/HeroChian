...
const Testgas = async()=>{
  ...
  setLoading(true)
   ...
   await useWrite(web3reactContext,[ERC721,TokenId,Pirce,endtime,isauction,1],"listToken",true);
   setLoading(false)
}
...
const Listed_Token =async()=>{
  ...
  setLoading(true)
   await useWrite(web3reactContext,[ERC721,TokenId,price,endtime,isauction,1],"listToken",true);
   setLoading(false);
   router.push("/Newlist")
}
...
const SetAPPROVED_ALL = async()=>{
  setLoading(true);
   await NFT_CONTRACT_APPROVE(web3reactContext,ERC721,"setApprovalForAll",[MARKET_CONTRACT,1]);
   setapprove(true)
   setLoading(false);
}
...
<h1 className='text-2xl font-bold pb-4'>List item for sale
</h1>
...
<button onClick={()=>setisauction(0)} className={`mt-2 w-full border-2  font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline ${isauction==0?"bg-blue-500 text-white":"text-black"} `}>Fixed Price</button>

</div>
<div className='flex-1'>
<button  onClick={()=>setisauction(1)} className={`mt-2 w-full border-2   font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline ${isauction==1?"bg-blue-500 text-white":"text-black"}`}>Auction</button>
...
<input onChange={(e)=>setPrice(e.target.value)} className='rounded-xl bg-white appearance-none border-2 border-gray-200  w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500"'></input>
...
<div className='mt-2'>
  {loaded &&   <ButtonLoad/> }
{!loaded && !approve &&     <button disabled={loading} onClick={()=>SetAPPROVED_ALL()} className='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline'>Approve </button>
 }
    {!loaded && approve && <button disabled={loading} onClick={()=>Listed_Token()} className='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline'>Complete Listing</button>}
</div>
