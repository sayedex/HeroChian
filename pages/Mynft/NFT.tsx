...
const NFT = () => {
  ...
  return (
    <div className="min-h-screen">
      ...
      {userFetchNFT === "LOADING" && (
        <div className="h-auto flex justify-center pt-5">
          <GridLoader color={"#000000"} loading={true} size={10} />
        </div>
      )}
      {userFetchNFT === "ERROR" && (
        <div className="h-auto flex justify-center pt-5">
      ...
      {userFetchNFT === "IDLE" && userNFT && userNFT.length == 0
        ? active && (
            <div className="text-center font-semibold text-red pt-10">
      ...
      {active && (
        <div className="max-w-7xl mt-10 m-auto flex justify-center flex-wrap gap-y-5 gap-x-2 pb-20">
          {userFetchNFT === "IDLE" &&
            userNFT &&
            userNFT.map((e: any, index: any) => {
              return <Nftcom key={index} Data={e} />;
            })}
        </div>
      )}
    </div>
