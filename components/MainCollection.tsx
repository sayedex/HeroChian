...
const Callmeta = async()=>{
    nfts &&  Getnftmetadata(`${ConvertLink(metadataUrl)}`).then((message:any) => { 
        setMeta(message)
        }).catch((message:any) => { 
    console.log(message);
...
<h2 className='font-semibold text-xl'>{numberTokensListed}</h2>}
    ...
    <div className=''>
{  !Data?<Skeleton  borderRadius={5} height={30} width={50} highlightColor="#b0a7a7" baseColor="#e7e7e7"/>: 
  <h2 className='font-semibold text-xl'>{dayData[0]?.dailyVolumeBNB==null?0:dayData[0]?.dailyVolumeBNB} ETH</h2> }
    <p className='font-light text-gray-500 text-sm'>Total volume</p>

    </div>
    <div>
{!Data?<Skeleton  borderRadius={5} height={30} width={50} highlightColor="#b0a7a7" baseColor="#e7e7e7"/>: 
    <h2 className='font-semibold text-xl'>{currentAskPrice && currentAskPrice} ETH</h2> }
