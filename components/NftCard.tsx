...
const NftCard = ({ nft, which }: props) => {
  ...
  return (
    <div
      className="bg-white	relative  m-5 p-2 rounded-lg object-contain	min-w-fit shadow-md bg-blend-multiply border hover:opacity-75		"
      id={style.main}
    >
      <Link
        href={`/collection/${Collectionaddress}/${tokenId}/${
          finalLink == 0 ? TypeNFT.newList : TypeNFT.auctionList
        }`}
      >
        {/* nft  */}
        ...
        <div className="max-w-[254px]">
          ...
          which == TypeNFT.slider ? (
            <div className="absolute flex gap-2 items-center top-4 left-4 py-2 px-4 bg-black bg-opacity-50 rounded-lg z-10 backdrop">
              <svg
                ...
                height="18"
              >
                <path d="M11.556 1.8a21.14 21.14 0 0 0-2.008 2.108A27.424 27.424 0 0 0 6 0C2.49 3.205 0 7.381 0 9.9 0 14.374 3.582 18 8 18s8-3.626 8-8.1c0-1.873-1.856-5.735-4.444-8.1Zm-.696 11.976a4.853 4.853 0 0 1-2.758.849c-2.577 0-4.673-1.678-4.673-4.403 0-1.358.868-2.554 2.6-4.597.247.28 3.529 4.406 3.529 4.406l2.094-2.35c.148.24.282.476.402.701.977 1.835.565 4.183-1.194 5.394Z"></path>
              </svg>
              <div className="font-semibold text-white">{which}</div>
            </div>
          ) : null}
          ...
          {/* listing price */}
          {which == TypeNFT.newList ? (
            <div className="text-base flex gap-1 items-center	font-medium font-sans">
              <FaEthereum />
              {nft && nft?.currentAskPrice} ETH
            </div>
          ) : (
            <div className="flex justify-between items-center border-t pt-2 pb-1 border-slate-400	">
              <h2 className="text-neutral-800  font-medium font-sans	text-xs">
                Asking price
              </h2>
