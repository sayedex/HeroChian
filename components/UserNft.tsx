...
const Nftcom = ({Data:{amount,metadata,token_address,token_id,token_uri,name}}:Props) => {
...
// console.log(Filter);
  ...
  return (<>
  {
    <div className="m-auto p-3 h-auto flex justify-center flex-col bg-white rounded-lg object-contain	w-fit shadow-md bg-blend-multiply border min-h-full	">
...
<h1 className='text-neutral-800		font-bold font-sans	text-lg	'>{name} #{token_id}</h1>
</div>
{/* name  */}
...
<div className="pt-3">
  ...
  <div>
     <Link href={`${token_address}/${token_id}`}><button onClick={()=>addTocard()} className="mt-2 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Sale</button></Link>
