...
import {
  ...
  increaseGasLimit,
  GetWeth,
} from "../hook/contractcall";
import { useRouter } from "next/router";
function useBalances(provider?: any, account?: any): BigNumber | undefined {
...
export const Listed_Token = async (
  ...
  //coming from hook
  const myContract = await getmainContract(
...
export const NFT_CONTRACT = async (
  ...
  const notification = toast.loading("Listing...");
  //coming from hook
  const myContract = await GetWeth(
    web3reactContext.library,
    web3reactContext.account
  );

  const gasprice = await myContract.estimateGas?.[name]({
    value: parseUnits(value),
  });
  console.log(gasprice);
